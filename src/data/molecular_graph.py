...
class MolecularGraphConverter:
    ...
    def __init__(self, 
        ...
        self.bond_feature_dim = (sum(self.bond_feature_dims.values())
                                 if use_bond_types else 0)
    ...
    def smiles_to_graph(self, smiles: str) -> Optional[Data]:
        ...
        try:
            ...
            if mol.GetNumAtoms() > self.max_atoms:
                logger.warning(f"Molecule too large: {mol.GetNumAtoms()} atoms (max: {self.max_atoms})")
                return None
    ...
    def _get_atom_features(self, mol: Chem.Mol) -> List[List[float]]:
        ...
        for atom in mol.GetAtoms():
            ...
            features.extend(self._one_hot_encode(atomic_num, self.atom_feature_dims['atomic_num']))
            
            # Degree (number of connections)
            degree = atom.GetDegree()
            features.extend(self._one_hot_encode(degree, self.atom_feature_dims['degree']))
            
            # Formal charge
            formal_charge = atom.GetFormalCharge() + 5  # Shift to 0-10 range
            features.extend(self._one_hot_encode(formal_charge, self.atom_feature_dims['formal_charge']))
            ...
            features.extend(self._one_hot_encode(hyb_idx, self.atom_feature_dims['hybridization']))
            
            # Number of hydrogens
            num_h = atom.GetTotalNumHs()
            features.extend(self._one_hot_encode(min(num_h, 4), self.atom_feature_dims['num_h']))
            
            # Aromaticity
            is_aromatic = 1 if atom.GetIsAromatic() else 0
            features.extend(self._one_hot_encode(is_aromatic, self.atom_feature_dims['aromatic']))
            ...
            if self.use_chirality:
                ...
                features.extend(self._one_hot_encode(chiral_idx, self.atom_feature_dims['chirality']))
            else:
    ...
    def _get_bond_features(self, mol: Chem.Mol) -> Tuple[List[List[int]], Optional[List[List[float]]]]:
        ...
        for bond in mol.GetBonds():
            ...
            if self.use_bond_types:
                ...
                edge_attr.extend([bond_features, bond_features])  # Same features for both directions
    ...
    def _get_single_bond_features(self, bond: Chem.Bond) -> List[float]:
        ...
        features.extend(self._one_hot_encode(bond_idx, self.bond_feature_dims['bond_type']))
        
        # Conjugation
        is_conjugated = 1 if bond.GetIsConjugated() else 0
        features.extend(self._one_hot_encode(is_conjugated, self.bond_feature_dims['conjugated']))
        
        # Ring membership
        is_in_ring = 1 if bond.IsInRing() else 0
        features.extend(self._one_hot_encode(is_in_ring, self.bond_feature_dims['in_ring']))
        ...
        features.extend(self._one_hot_encode(stereo_idx, self.bond_feature_dims['stereo']))
