...
class EnsembleGCN:
    ...
    def predict(
        self, dataloader, return_variance: bool = True
...
class UncertaintyCalibrator:
    ...
    def calibrate(
        ...
        targets: np.ndarray,
        confidence_levels: List[float] = [0.5, 0.68, 0.95, 0.99],
    ) -> Dict:
        ...
        results["error_correlation"]["spearman"] = stats.spearmanr(
            errors.flatten(), uncertainties.flatten()
        )[0]
        ...
        for conf_level in confidence_levels:
            ...
            if len(targets.shape) > 1:
                ...
                coverage = np.mean(
                    (targets >= lower_bound.flatten())
                    & (targets <= upper_bound.flatten())
                )
        ...
        results["outlier_detection"] = {
            ...
            "outlier_mae": (
                np.mean(errors[high_uncertainty_mask])
...
class UncertaintyQuantifier:
    ...
    def __init__(
        self, models: List[nn.Module], device: torch.device, mc_samples: int = 20
    ):
    ...
    def predict_with_uncertainty(
        self, dataloader, method: str = "ensemble"
        ...
        if method == "ensemble":
            ...
            logger.warning(
                "MC Dropout not yet implemented for dataloader. Using ensemble."
            )
            return self.ensemble.predict(dataloader, return_variance=True)
    ...
    def evaluate_uncertainty(
        ...
        results_dir: Path = Path("results/uq"),
    ) -> Dict:
        ...
        logger.info(
            f"   Error-Uncertainty Correlation: {calibration_results['error_correlation']['pearson']:.3f}"
        )
        logger.info(
            f"   95% Coverage: {calibration_results['coverage'][-2]:.3f} (Expected: 0.95)"
        )

        return results


def create_ensemble_from_hpo_results(
    hpo_results: Dict, dataset, device: torch.device, top_k: int = 5
) -> List[nn.Module]:
