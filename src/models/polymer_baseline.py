...
class PolymerFingerprintBaseline(nn.Module):
    ...
    def forward(self, fingerprints: torch.Tensor, chain_features: torch.Tensor) -> torch.Tensor:
        """
...
class PolymerFeatureExtractor:
        ...
        try:
            ...
            if self.fingerprint_type == 'morgan':
                ...
                if len(fp_array) < self.fingerprint_dim:
                    fp_array = np.pad(fp_array, (0, self.fingerprint_dim - len(fp_array)))
                else:
    ...
    def extract_chain_features(self, smiles: str) -> np.ndarray:
        ...
        try:
            ...
            logger.warning(f"Failed to extract chain features from SMILES {smiles}: {e}")
            return np.zeros(10)
...
class PolymerFingerprintDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        logger.info(f"Feature extraction complete. Fingerprints: {self.fingerprints.shape}, "
    ...
    def __getitem__(self, idx):
        return {
            ...
            'chain_features': torch.tensor(self.chain_features[idx], dtype=torch.float32),
            'target': torch.tensor(self.targets[idx], dtype=torch.float32),
            'smiles': self.smiles_list[idx]
        }


def create_baseline_model_and_extractor(config: Dict) -> Tuple[PolymerFingerprintBaseline, PolymerFeatureExtractor]:
    """
