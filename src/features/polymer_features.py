...
def calculate_molecular_weight(smiles: str) -> float:
    ...
    try:
        ...
        if cleaned_mol is None:
            raise PolymerFeatureError(
                f"Failed to process cleaned molecule from: {smiles}"
            )
        ...
        raise PolymerFeatureError(
            f"Error calculating molecular weight for {smiles}: {str(e)}"
        )


def encode_degree_polymerization(
    dp: Union[float, int], max_dp: int = 10000, log_scale: bool = True
) -> np.ndarray:
    """
...
def extract_repetition_unit_features(
    smiles: str, fingerprint_size: int = 128, radius: int = 2
) -> np.ndarray:
    """
    ...
    try:
        ...
        with suppress_rdkit_warnings():
            try:
                fp = AllChem.GetMorganFingerprintAsBitVect(
                    unit_mol, radius=radius, nBits=fingerprint_size
                )
        ...
        raise PolymerFeatureError(
            f"Error extracting unit features for {smiles}: {str(e)}"
        )


def calculate_chain_length_descriptors(
    smiles: str, dp: Union[float, int] = 1
) -> np.ndarray:
    """
    ...
    try:
        ...
        ring_count = (
            rdMolDescriptors.CalcNumRings(unit_mol) if unit_mol.GetNumAtoms() > 0 else 0
        )
        rigidity_factor = 1.0 + ring_count * 2.0  # Rings increase rigidity
        flexibility_factor = (
            1.0 / (1.0 + rotatable_bonds) if rotatable_bonds > 0 else 1.0
        )
        persistence_length_est = (
            rigidity_factor * flexibility_factor * 10.0
        )  # Scaled estimate
        ...
        chain_descriptors = np.array(
            [
                ...
                persistence_length_est,
                (
                    np.log10(end_to_end_est) if end_to_end_est > 0 else 0
        ...
        raise PolymerFeatureError(
            f"Error calculating chain descriptors for {smiles}: {str(e)}"
        )
...
def calculate_repetition_unit_complexity(smiles: str) -> np.ndarray:
    ...
    try:
        ...
        ring_complexity = float(
            num_rings + 0.5 * num_aromatic_rings
        )  # Aromatic rings add complexity

        # 2. Heteroatom ratio
        hetero_atoms = sum(
            1 for atom in unit_mol.GetAtoms() if atom.GetAtomicNum() not in [1, 6]
        )
        ...
        degrees = [
            atom.GetDegree() for atom in unit_mol.GetAtoms() if atom.GetAtomicNum() != 1
        ]
        ...
        stereo_centers = len(
            Chem.FindMolChiralCenters(unit_mol, includeUnassigned=True)
        )
        stereo_bonds = sum(
            1
            for bond in unit_mol.GetBonds()
        ...
        raise PolymerFeatureError(
            f"Error calculating unit complexity for {smiles}: {str(e)}"
        )


def calculate_polymer_molecular_descriptors(
    smiles: str, dp: Union[float, int] = 1
) -> np.ndarray:
    """
    ...
    try:
        ...
        try:
            ...
            free_volume_fraction = (
                max(0, (molar_volume - vdw_volume) / molar_volume)
                if molar_volume > 0
                else 0
            )
        except Exception:
        ...
        stiffness = float(rings * 2.0 + aromatic_rings * 3.0) / max(
            1, rotatable_bonds + 1
        )
        ...
        try:
            ...
            interaction_strength = (polar_surface_area / 100.0) + (
                hbd_count + hba_count
            ) * 0.5
        except Exception:
        ...
        try:
            ...
            heavy_atoms = [
                atom for atom in unit_mol.GetAtoms() if atom.GetAtomicNum() > 1
            ]
            if len(heavy_atoms) > 2:
                ...
                shape_factor = (
                    actual_bonds / max_possible_bonds if max_possible_bonds > 0 else 0.5
                )
            else:
                shape_factor = 0.5
        except Exception:
        ...
        branching_ratio = (
            sum(
                1
                for atom in unit_mol.GetAtoms()
                if atom.GetDegree() > 2 and atom.GetAtomicNum() != 1
            )
            / num_atoms
        )
        ...
        try:
            polarity_factor = polar_surface_area / (
                molecular_weight + 1
            )  # Normalized polarity
        except Exception:
            polarity_factor = 0.0
        ...
        symmetry_factor = (
            float(aromatic_rings + rings) / num_atoms if num_atoms > 0 else 0
        )
        regularity_factor = 1.0 / (
            1.0 + branching_ratio * 3.0
        )  # Less branching = more regular
        ...
        raise PolymerFeatureError(
            f"Error calculating polymer descriptors for {smiles}: {str(e)}"
        )


def extract_polymer_features(
    smiles: str,
    ...
    try:
        ...
        if dp is None and mw is not None:
            ...
            if not hasattr(extract_polymer_features, "_dp_warning_logged"):
                logger.info(
                    "DP and MW values not provided - using default DP=1.0 for all molecules"
                )
        ...
        raise PolymerFeatureError(
            f"Error extracting polymer features for {smiles}: {str(e)}"
        )
...
class PolymerFeatureExtractor:
    ...
    def __init__(
        ...
        if include_molecular_descriptors:
            self.feature_dim += (
                7  # Polymer descriptors (updated count after tg_predictor removal)
            )

        logger.info(
            f"Initialized PolymerFeatureExtractor with {self.feature_dim} features"
        )
    ...
    def extract_features(
        ...
        cache_key = (
            (
                f"{smiles}_{dp}_{mw}_{self.include_chain_descriptors}_"
                f"{self.include_complexity}_{self.include_molecular_descriptors}"
            )
