...
class PolymerBaselineTrainer:
    ...
    def train(self, 
        ...
        if val_dataset is None:
            ...
            logger.info(f"Split dataset: {train_size} train, {val_size} validation samples")
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        for key, value in results.items():
            if isinstance(value, dict):
                ...
                serializable_results[key] = float(value) if isinstance(value, (int, float, np.number)) else value
        
        with open(self.results_dir / f"{self.model_name}_results.json", 'w') as f:
            json.dump(serializable_results, f, indent=2)
        
        logger.info(f"Results saved to {self.results_dir / f'{self.model_name}_results.json'}")
    ...
    def _plot_training_curves(self):
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / f"{self.model_name}_training_curves.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info(f"Training curves saved to {self.results_dir / f'{self.model_name}_training_curves.png'}") 