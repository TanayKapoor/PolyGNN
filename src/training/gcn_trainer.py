            ...
            for metric, value in metrics.items():
                if metric.lower() == "r²":
                    status = (
                        "✅ Excellent"
                        if value >= 0.7
                        else "✅ Good" if value >= 0.5 else "⚠️ Poor"
                    )
                elif "rmse" in metric.lower():
                    status = (
                        "✅ Excellent"
                        if value <= 40
                        else "✅ Good" if value <= 60 else "⚠️ High"
                    )
                elif "mae" in metric.lower():
                    status = (
                        "✅ Excellent"
                        if value <= 25
                        else "✅ Good" if value <= 40 else "⚠️ High"
                    )
                else:
                    status = ""
...
class PolymerGCNTrainer:
    ...
    def _create_epoch_display(
        ...
        if RICH_AVAILABLE:
            ...
            progress_bar = "█" * int(20 * epoch / max_epochs) + "░" * (
                20 - int(20 * epoch / max_epochs)
            )
...
"""
    ...
    def train(
        ...
        if has_polymer_features:
            beautiful.print_info(
                f"Using polymer features: dimension {sample.polymer_features.shape[0]}"
            )
        else:
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if epoch == 1 or epoch % 10 == 0 or epoch == max_epochs:
                ...
                if RICH_AVAILABLE:
                    ...
                    beautiful.console.print(
                        Panel(display, title=f"[bold]Epoch {epoch}[/bold]"), end=""
                    )
                else:
                    print(
                        self._create_epoch_display(
                            epoch,
                            max_epochs,
                            train_metrics,
                            val_display_metrics,
            ...
            if val_metrics["r2"] > best_val_r2:
                ...
                if epoch > 1 and (improvement > 0.01 or epoch % 10 == 0):
                    beautiful.print_success(
                        f"New best validation R²: {val_metrics['r2']:.4f}"
                    )
        ...
        if save_name:
            ...
            beautiful.print_success(
                f"Training curves saved to {self.results_dir / f'{save_name}_training_curves.png'}"
            )
    ...
    def _train_epoch(
        self, train_loader, optimizer, criterion
    ...
    def _calculate_metrics(
        self, predictions: List[float], targets: List[float], loss: float
    ) -> Dict[str, float]:
    ...
    def _plot_training_curves(
        self, train_losses, val_losses, train_metrics, val_metrics
    ):
    ...
    def cross_validate(
        ...
        logger.info(
            f"Mean R²: {cv_results['mean_r2']:.4f} ± {cv_results['std_r2']:.4f}"
        )
    ...
    def _calculate_cv_statistics(self, fold_results: List[Dict]) -> Dict[str, Any]:
        ...
        cv_results["fold_scores"] = {
            metric: [fold[metric] for fold in fold_results] for metric in metrics
        }

        return cv_results

    def hyperparam_optimize(
        self,
        dataset,
        ...
        if method == "grid":
            ...
            beautiful.print_info(
                f"Grid search: {len(param_combinations)} total combinations"
            )
        elif method == "random":
            param_combinations = self._generate_random_combinations(
                param_grid, n_trials, random_seed
            )
            ...
            def process_trial(trial_id, params, progress_callback=None):
                ...
                try:
                    ...
                    if is_best:
                        ...
                        beautiful.print_success(
                            f"NEW BEST! {primary_metric}: {current_score:.4f}"
                        )
            ...
            if RICH_AVAILABLE:
                with Progress(
                    ...
                    task = progress.add_task(
                        "HPO Progress", total=len(param_combinations)
                    )
                ...
                for trial_id, params in enumerate(
                    tqdm(param_combinations, desc="HPO Progress")
                ):
        ...
        if all_results:
            ...
            if RICH_AVAILABLE:
                ...
                best_table = Table(
                    title=f"Best Parameters ({primary_metric}: {best_score:.4f})"
                )
                ...
                beautiful.print_section(
                    f"Best Parameters ({primary_metric}: {best_score:.4f})"
                )
                for param, value in best_params.items():
                    print(f"  {param}: {value}")
        else:
    ...
    def _generate_random_combinations(
        self, param_grid: Dict[str, List], n_trials: int, random_seed: int
    ) -> List[Dict]:
    ...
    def _run_hpo_trial(
        ...
        try:
            sample = (
                dataset[0] if hasattr(dataset, "__getitem__") else next(iter(dataset))
            )
            ...
            logger.warning(
                f"Could not determine node feature dimension from dataset: {e}, using fallback 157"
            )
        ...
        if save_model or cv_results["mean_r2"] > getattr(
            self, "_best_trial_r2", -np.inf
        ):
    ...
    def retrain_with_best_params(
        ...
        if RICH_AVAILABLE:
            ...
            retrain_table = Table(
                title=f"Final Training Configuration (CV {primary_metric}: {best_cv_score:.4f})"
            )
            ...
            beautiful.print_section(
                f"Final Training Configuration (CV {primary_metric}: {best_cv_score:.4f})"
            )
            for param, value in best_params.items():
                print(f"  {param}: {value}")
        ...
        sample = (
            full_dataset[0]
            if hasattr(full_dataset, "__getitem__")
            else next(iter(full_dataset))
        )
        node_feature_dim = sample.x.shape[1] if hasattr(sample, "x") else 157
        ...
        # Final training
        logger.info(
            f"Final training: {len(train_subset)} train, {len(val_subset)} validation"
        )
        ...
        if test_dataset is not None:
            ...
            if all_success:
                ...
                logger.info(
                    f"   ✅ R² ≥ {success_criteria['r2_target']}: {test_metrics['r2']:.4f}"
                )
                logger.info(
                    f"   ✅ RMSE ≤ {success_criteria['rmse_target']}: {test_metrics['rmse']:.4f}"
                )
                logger.info(
                    f"   ✅ MAE ≤ {success_criteria['mae_target']}: {test_metrics['mae']:.4f}"
                )
            else:
                logger.info("⚠️  Some criteria not met:")
                logger.info(
                    f"   {'✅' if r2_success else '❌'} R² ≥ {success_criteria['r2_target']}: {test_metrics['r2']:.4f}"
                )
                logger.info(
                    f"   {'✅' if rmse_success else '❌'} RMSE ≤ {success_criteria['rmse_target']}: {test_metrics['rmse']:.4f}"
                )
                logger.info(
                    f"   {'✅' if mae_success else '❌'} MAE ≤ {success_criteria['mae_target']}: {test_metrics['mae']:.4f}"
                )
