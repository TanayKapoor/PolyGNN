...
class ExternalValidator:
    ...
    def load_external_data(self, file_path: str) -> pd.DataFrame:
        ...
        exclude_cols = ['smiles', 'canonical_smiles', 'SMILES', 'Tg', 'Tm', 'Density', 'FFV', 'Tc', 'Rg', 'source']
    ...
    def predict_with_ensemble(self, graphs: List[Data]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def calculate_metrics(self, true_values: np.ndarray, predictions: np.ndarray, 
                         uncertainties: np.ndarray, property_name: str) -> Dict:
        ...
        unc_corr = np.corrcoef(errors, unc_clean)[0, 1] if len(unc_clean) > 1 else np.nan
    ...
    def robustness_test(self, df: pd.DataFrame, graphs: List[Data], 
        ...
        if self.feature_cols and len(self.feature_cols) > 0:
            ...
            if len(numeric_feats) > 0:
                ...
                df_noise[numeric_feats] = df_noise[numeric_feats] + df_noise[numeric_feats] * noise
                ...
                if len(graphs_noise) > 0:
                    ...
                    # Calculate shifts
                    pred_shift = np.mean(np.abs(orig_preds - noise_preds)) / (np.mean(np.abs(orig_preds)) + 1e-8) * 100
    ...
    def run_validation(self, external_file: str, output_file: str = "data/processed/external_preds.csv"):
        ...
        for prop in target_props:
            if prop in df_val.columns:
                ...
                metrics = self.calculate_metrics(true_vals, predictions, uncertainties, prop)
