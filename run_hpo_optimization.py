...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('hpo_optimization.log'),
...
def define_search_space(method: str = 'grid'):
    ...
    if method == 'grid':
        ...
        if total_combinations > 1000:
            logger.warning(f"Large search space ({total_combinations:,} combinations)! Consider using random search.")
...
def load_data(data_path: str = "data/processed/filtered_tg_dataset.csv") -> tuple:
    ...
    if len(combined_data) > 0:
        ...
        logger.info(f"  Node features: {sample.x.shape[1] if hasattr(sample, 'x') else 'N/A'}")
        logger.info(f"  Molecular features: {sample.mol_features.shape[0] if hasattr(sample, 'mol_features') else 'N/A'}")
        logger.info(f"  Polymer features: {sample.polymer_features.shape[0] if hasattr(sample, 'polymer_features') else 'N/A'}")
...
def run_hpo(args):
    ...
    logger.info(f"Successful trials: {hpo_results['successful_trials']}/{hpo_results['total_trials']}")
    ...
    if 'test_metrics' in final_results:
        ...
        if 'success_criteria' in final_results:
            ...
            logger.info(f"  R² ≥ 0.5: {'✅' if success['r2'] else '❌'} ({test_metrics['r2']:.4f})")
            logger.info(f"  RMSE ≤ 50.0: {'✅' if success['rmse'] else '❌'} ({test_metrics['rmse']:.4f})")
            logger.info(f"  MAE ≤ 30.0: {'✅' if success['mae'] else '❌'} ({test_metrics['mae']:.4f})")
            logger.info(f"  Overall: {'🎉 SUCCESS!' if success['overall'] else '⚠️  Partially successful'}")
...
def main():
    parser = argparse.ArgumentParser(description='Hyperparameter Optimization for Polymer GCN')
    ...
    parser.add_argument('--primary_metric', type=str, default='r2', choices=['r2', 'rmse', 'mae'],
                       help='Primary metric for optimization')
    
    # Training parameters
    parser.add_argument('--max_epochs', type=int, default=50,
                       help='Maximum epochs per HPO trial')
    parser.add_argument('--patience', type=int, default=8,
                       help='Early stopping patience for HPO trials')
    ...
    parser.add_argument('--data_path', type=str, default='data/processed/filtered_tg_dataset.csv',
