            ...
            try:
                ...
                if 'model_config' in checkpoint:
                    ...
                    architectures = [
                        ...
                        {'node_feature_dim': 157, 'hidden_dims': [256, 128], 'num_gcn_layers': 3, 'dropout_rate': 0.3, 'polymer_feature_dim': 147},
                        # Enhanced model
                        {'node_feature_dim': 157, 'hidden_dims': [128, 64, 32], 'num_gcn_layers': 3, 'dropout_rate': 0.2},
                        # Baseline model
                        {'node_feature_dim': 157, 'hidden_dims': [128, 64, 32], 'num_gcn_layers': 3, 'dropout_rate': 0.2, 'use_polymer_features': False}
                    ]
                    ...
                    for arch in architectures:
                        try:
                            ...
                            logger.info(f"✅ Successfully loaded with architecture: {arch}")
                            break
    ...
    if 'Tg' in df_valid.columns:
        ...
        if len(true_clean) > 0:
            ...
            if r2 < 0.6:
                ...
                logger.info(f"   R² = {r2_improved:.3f} (improved by +{r2_improved - r2:.3f})")
                logger.info(f"   RMSE = {rmse_improved:.2f}°C (improved by -{rmse - rmse_improved:.2f})")
                logger.info(f"   MAE = {mae_improved:.2f}°C (improved by -{mae - mae_improved:.2f})")
