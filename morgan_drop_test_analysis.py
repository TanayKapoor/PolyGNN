...
def run_morgan_drop_test():
    ...
    print(f'   Slim feature set: {feature_extractor.get_feature_dim()} features (Morgan FPs excluded)')
    ...
    print(f'   R² change: {test_r2:.4f} vs {full_run_r2:.4f} = {test_r2 - full_run_r2:+.4f}')
    print(f'   RMSE change: {test_rmse:.2f} vs {full_run_rmse:.2f} = {test_rmse - full_run_rmse:+.2f}°C')
    print(f'   MAE change: {test_mae:.2f} vs {full_run_mae:.2f} = {test_mae - full_run_mae:+.2f}°C')
    ...
    for idx, row in importance_df.iterrows():
        ...
        elif any(desc in name for desc in ['logp', 'tpsa', 'hbd', 'hba', 'rotatable', 'aromatic']):
    ...
    for group, data in group_importance.iterrows():
        print(f'   {group}: {data["total_importance"]:.4f} (n={int(data["feature_count"])})')
    
    print('\n🏆 Top 10 Features (Morgan FPs excluded):')
    for i, (_, row) in enumerate(importance_df.head(10).iterrows(), 1):
        print(f'   {i:2d}. {row["feature_name"]:25s} | {row["importance_mean"]:8.4f} | {row["feature_group"]}')
    
    # Calculate efficiency metrics
    total_importance = importance_df['importance_mean'].sum()
    cumulative_importance = np.cumsum(importance_df['importance_mean']) / total_importance
    features_80_pct = np.where(cumulative_importance >= 0.8)[0][0] + 1
    features_90_pct = np.where(cumulative_importance >= 0.9)[0][0] + 1
    
    print(f'\n💡 Feature Selection Insights:')
    print(f'   80% importance captured by top {features_80_pct} features ({features_80_pct/20*100:.1f}% of slim set)')
    print(f'   90% importance captured by top {features_90_pct} features ({features_90_pct/20*100:.1f}% of slim set)')
...
'''
    ...
    chain_desc_after = group_importance.loc['Chain Descriptors', 'total_importance'] if 'Chain Descriptors' in group_importance.index else 0
