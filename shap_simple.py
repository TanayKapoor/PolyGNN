...
class SimplePolymerSHAP:
    ...
    def _prepare_data(self) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        skip_cols = ['canonical_smiles', 'smiles', 'Tg', 'Tm', 'Density', 'FFV', 'Tc', 'Rg', 'source']
        ...
        logger.info(f"📊 Numeric conversion: {numeric_features.notna().sum().sum()} valid values")
        ...
        if len(features) > 0:
            ...
            logger.info(f"📊 Feature range: {features.min():.3f} to {features.max():.3f}")
            logger.info(f"🎯 Target range: {targets.min():.1f}°C to {targets.max():.1f}°C")
    ...
    def run_shap_analysis(self, model: RandomForestRegressor, n_samples: int = 200) -> Dict:
    ...
    def _analyze_shap_results(self, shap_values: np.ndarray, features: np.ndarray, 
                             targets: np.ndarray, model) -> Dict:
    ...
    def _categorize_features(self, top_features: List[Tuple[str, float]]) -> Dict:
        ...
        for feat_name, importance in top_features:
            ...
            if any(keyword in name_lower for keyword in ['molecular', 'logp', 'tpsa', 'hbd', 'hba', 'rotatable']):
                categories['molecular_descriptors'].append({'name': feat_name, 'importance': importance})
            elif any(keyword in name_lower for keyword in ['fp_bit', 'fingerprint', 'ecfp', 'morgan']):
                categories['fingerprint_bits'].append({'name': feat_name, 'importance': importance})
            elif any(keyword in name_lower for keyword in ['dp', 'mw', 'polymer', 'chain', 'flex', 'degree']):
                categories['polymer_properties'].append({'name': feat_name, 'importance': importance})
            elif any(keyword in name_lower for keyword in ['aromatic', 'ring', 'bond', 'atom']):
                categories['structural_features'].append({'name': feat_name, 'importance': importance})
            else:
                categories['other'].append({'name': feat_name, 'importance': importance})
        
        return categories
    
    def create_visualizations(self, shap_values: np.ndarray, features: np.ndarray, 
                            results: Dict):
        ...
        display_names = [name[:30] + '...' if len(name) > 30 else name for name in names]
        
        bars = plt.barh(range(len(display_names)), importances, alpha=0.8)
        plt.yticks(range(len(display_names)), display_names)
        plt.xlabel('Mean |SHAP Value|')
        plt.title('Top 20 Features by SHAP Importance for Tg Prediction', fontsize=14, pad=20)
        ...
        plt.title('SHAP Summary Plot - Polymer Features for Tg Prediction', fontsize=14, pad=20)
        plt.tight_layout()
        ...
        if category_counts:
            ...
            plt.title('Distribution of Top 30 Features by Category', fontsize=14, pad=20)
            plt.axis('equal')
        ...
        plt.hist(shap_magnitude, bins=30, alpha=0.7, color='lightcoral', edgecolor='black')
...
def main():
    parser = argparse.ArgumentParser(description='Simple SHAP Analysis for Polymer Features')
