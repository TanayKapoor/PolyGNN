...
class CompletePolymerPreprocessor:
    ...
    def calculate_comprehensive_features(self, smiles):
        ...
        try:
            ...
            try:
                ...
                features["ETA_dBeta"] = (
                    Descriptors.NumAliphaticCarbocycles(mol) / features["MW"]
                    if features["MW"] > 0
                    else 0
                )
            ...
            features["chain_flexibility"] = (
                rotatable_bonds / num_atoms if num_atoms > 0 else 0
            )
            features["persistence_length_est"] = 1.0 / (
                features["chain_flexibility"] + 1e-6
            )
            features["chain_stiffness_log"] = np.log(
                features["persistence_length_est"] + 1
            )

            # Structural complexity
            features["ring_complexity"] = (
                num_rings * 2.0 / num_atoms if num_atoms > 0 else 0
            )
            heteroatoms = sum(
                1 for atom in mol.GetAtoms() if atom.GetSymbol() not in ["C", "H"]
            )
            features["heteroatom_ratio"] = (
                heteroatoms / num_atoms if num_atoms > 0 else 0
            )
            ...
            # Branching
            branch_points = sum(
                1 for atom in mol.GetAtoms() if len(atom.GetNeighbors()) > 2
            )
            features["branching_factor"] = (
                branch_points / num_atoms if num_atoms > 0 else 0
            )

            # Stereochemistry
            chiral_centers = len(Chem.FindMolChiralCenters(mol, includeUnassigned=True))
            features["stereochemical_complexity"] = (
                chiral_centers / num_atoms if num_atoms > 0 else 0
            )

            # Aromaticity
            aromatic_atoms = sum(1 for atom in mol.GetAtoms() if atom.GetIsAromatic())
            features["aromaticity_index"] = (
                aromatic_atoms / num_atoms if num_atoms > 0 else 0
            )

            # Volume and packing
            molecular_volume = features["MW"] / 0.6
            van_der_waals_volume = Descriptors.LabuteASA(mol)
            features["free_volume_fraction"] = (
                max(0, 1 - (van_der_waals_volume / molecular_volume))
            ...
            features["interaction_strength"] = (
                features["TopoPSA(NO)"] / features["MW"] if features["MW"] > 0 else 0
            )

            # Derived properties
            features["flexibility_factor"] = min(
                1.0, features["chain_flexibility"] * 10
            )
            features["polarity_factor"] = features["TopoPSA(NO)"] / 100.0
            features["crystallinity_indicator"] = max(
                0,
                min(
                    1,
                    1.0 - features["branching_factor"] - features["chain_flexibility"],
                ),
            )

            # Geometric properties (log-scaled)
            features["end_to_end_distance_log"] = np.log(
                features["degree_polymerization"] * features["persistence_length_est"]
                + 1
            )
            features["radius_gyration_log"] = features["end_to_end_distance_log"] - 0.5
            features["chain_compactness_log"] = -features[
                "end_to_end_distance_log"
            ] + np.log(features["MW"] + 1)
    ...
    def preprocess_dataset(self, input_file, output_file):
        ...
        logger.info(
            f"❌ Invalid SMILES: {invalid_count} ({invalid_count/len(df)*100:.1f}%)"
        )
        ...
        logger.info(
            f"🧪 Calculating {len(self.feature_names)} comprehensive features..."
        )
        ...
        column_order = (
            ["smiles", "Tg", "canonical_smiles", "source"]
        ...
        logger.info(
            f"🧪 Features calculated: {len([col for col in result_df.columns if col in self.feature_names])}"
        )
        ...
        if "Tg" in result_df.columns and result_df["Tg"].notna().sum() > 0:
            ...
            try:
                ...
                if len(tg_clean) > 0:
                    logger.info(
                        f"   Range: {tg_clean.min():.1f}°C to {tg_clean.max():.1f}°C"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Complete polymer dataset preprocessing"
    )
    parser.add_argument("--input", type=str, required=True, help="Input dataset file")
    parser.add_argument(
        "--output",
        type=str,
        default="data/processed/external_validation.csv",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        result = preprocessor.preprocess_dataset(
            "test_polymer_data.csv", "results/external_validation_test.csv"
        )
    else:
        main()
