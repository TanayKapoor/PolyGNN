...
class ExpandedHPOOptimizer:
    ...
    def _prepare_dataset(self):
        ...
        available_targets = [
            col
            for col in ["Tg", "Tm", "Density", "FFV", "Tc", "Rg"]
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        params = {
            ...
            "polymer_feature_weight": trial.suggest_float(
                "polymer_feature_weight", 0.5, 2.0
            ),
        }
    ...
    def _train_and_evaluate(self, params: Dict[str, Any], trial_number: int) -> float:
        ...
        try:
            ...
            split_ratios = (
                config["data"]["splits"]["train_ratio"],
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        results_file = (
            self.results_dir
            / f"expanded_hpo_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        study_file = (
            self.results_dir
            / f"optuna_study_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        )
        import pickle
...
def main():
    ...
    print(
        f"✅ Target R² ≥ 0.5: {'ACHIEVED' if results['target_achieved'] else 'NOT YET'}"
    )
