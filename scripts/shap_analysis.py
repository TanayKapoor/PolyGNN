...
class PolyGNNSHAPAnalyzer:
    ...
    def extract_polymer_features(self, dataset) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.info(
            f"📊 Feature range: {polymer_features.min():.3f} to {polymer_features.max():.3f}"
        )

        return polymer_features, targets

    def create_model_wrapper(self, model: nn.Module, template_dataset) -> callable:
        ...
        def predict_from_polymer_features(polymer_features: np.ndarray) -> np.ndarray:
            ...
            with torch.no_grad():
                for i in range(0, n_samples, batch_size):
                    ...
                    # Convert to tensor
                    batch_features_tensor = torch.FloatTensor(batch_features).to(
                        self.device
                    )
    ...
    def run_shap_analysis(
        ...
        background_sample = shap.kmeans(
            background_features, min(10, len(background_features))
        )
    ...
    def _create_shap_visualizations(
        self, shap_values: np.ndarray, features: np.ndarray, targets: np.ndarray
    ):
        ...
        shap.plots.beeswarm(
            shap.Explanation(
                values=shap_values, data=features, feature_names=self.feature_names
            ),
            max_display=20,
            show=False,
        )
    ...
    def print_analysis_summary(self, results: Dict[str, Any]):
        ...
        for i, feat in enumerate(top_features[:10]):
            print(
                f"   {i+1:2d}. {feat['name']:<30} | Importance: {feat['importance']:.4f}"
            )

        print(f"\n📈 FEATURE STATISTICS:")
        print(f"   Mean SHAP magnitude: {stats['mean_shap_magnitude']:.4f}")
        print(
            f"   SHAP value range: {stats['min_shap_value']:.4f} to {stats['max_shap_value']:.4f}"
        )
        ...
        print(
            f"   Tg range: {pred_stats['target_range'][0]:.1f}°C to {pred_stats['target_range'][1]:.1f}°C"
        )
        ...
        if molecular_features:
            print(
                f"   🧬 Key molecular descriptors: {len(molecular_features)} in top 20"
            )
            print(
                f"      Examples: {', '.join([f['name'] for f in molecular_features[:3]])}"
            )

        if structural_features:
            print(
                f"   🏗️  Key structural features: {len(structural_features)} in top 20"
            )
            print(
                f"      Examples: {', '.join([f['name'] for f in structural_features[:3]])}"
            )

        if polymer_features:
            print(f"   🔗 Key polymer properties: {len(polymer_features)} in top 20")
            print(
                f"      Examples: {', '.join([f['name'] for f in polymer_features[:3]])}"
            )

        print(f"\n💡 INSIGHTS:")
        print(
            f"   • SHAP analysis reveals which of the 148 features drive Tg predictions"
        )
        print(f"   • Feature importance guides future feature engineering")
        print(
            f"   • Validates domain knowledge about polymer structure-property relationships"
        )
...
def main():
    ...
    parser.add_argument(
        "--model_path", type=str, default=None, help="Path to trained model (optional)"
    )
    parser.add_argument(
        "--n_background",
        type=int,
    ...
    parser.add_argument(
        "--n_explain", type=int, default=100, help="Number of samples to explain"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    analyzer = PolyGNNSHAPAnalyzer(
        dataset_path=args.dataset, results_dir=args.results_dir, device=args.device
    )
