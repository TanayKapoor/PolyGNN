...
def analyze_feature_groups(importance_df):
    ...
    feature_groups = {
        "Molecular Weight": lambda x: "molecular_weight" in x.lower()
        or "mw" in x.lower(),
        "Degree of Polymerization": lambda x: "degree_polymerization" in x.lower()
        or "dp" in x.lower(),
        "Morgan Fingerprint": lambda x: "morgan_fp" in x.lower()
        or "fp_bit" in x.lower(),
        "Chain Descriptors": lambda x: "chain" in x.lower()
        or "flexibility" in x.lower(),
        "Molecular Descriptors": lambda x: any(
            desc in x.lower()
...
def create_visualizations(
    ...
    group_importance = (
        importance_df.groupby("feature_group")["importance_mean"]
        .sum()
    ...
    cumulative_importance = np.cumsum(importance_df["importance_mean"]) / np.sum(
        importance_df["importance_mean"]
    )
    ax5.plot(
        range(len(cumulative_importance)), cumulative_importance, "b-", linewidth=2
    )
...
"""
    ...
    features_80_pct = (
        np.where(
            np.cumsum(importance_df["importance_mean"])
            / np.sum(importance_df["importance_mean"])
            >= 0.8
        )[0][0]
        + 1
    )
...
def main():
    ...
    print(
        f'   • 80% importance captured by top {np.where(np.cumsum(importance_df["importance_mean"]) / np.sum(importance_df["importance_mean"]) >= 0.8)[0][0] + 1} features'
    )
