    ...
    def calc_features(row):
        ...
        try:
            ...
            features["chain_flexibility"] = (
                features["NumRotatableBonds"] / features["NumAtoms"]
                if features["NumAtoms"] > 0
                else 0
            )
            features["degree_polymerization"] = features["NumAtoms"] / 10.0
            features["ring_complexity"] = (
                features["NumRings"] / features["NumAtoms"]
                if features["NumAtoms"] > 0
                else 0
            )
...
if __name__ == "__main__":
    ...
    df = preprocess_polymer_dataset(
        "test_polymer_data.csv", "results/test_preprocessing/processed.csv"
    )
    print("\n✅ Preprocessing complete!")
    print(f"📊 Final dataset: {len(df)} samples, {len(df.columns)} columns")
    print(
        f"🧪 Sample features: {[col for col in df.columns if 'morgan_fp' in col][:5]}"
    )
