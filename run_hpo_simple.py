...
class SimpleHPOOptimizer:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        config['model']['hidden_dims'] = [params['hidden_dims']] * params['num_gcn_layers']
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        def progress_callback(study, trial):
            if trial.number % 5 == 0:
                current_val = f"{trial.value:.4f}" if trial.value is not None else "Failed"
                logger.info(f"📊 Progress: Trial {trial.number}/{self.n_trials} | "
        ...
        # Save results
        results_file = self.results_dir / f"hpo_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument('--results_dir', type=str, default='results/simple_hpo',
    ...
    print(f"✅ Target R² ≥ 0.5: {'ACHIEVED' if results['target_achieved'] else 'APPROACHING'}")
