...
from src.models.uncertainty_quantification import UncertaintyQuantifier, EnsembleGCN
...
class Week2Pipeline:
        ...
        try:
            ...
            self.final_results = {
                ...
                'summary': {
                    ...
                    'uncertainty_correlation': uq_results.get('calibration', {}).get('error_correlation', {}).get('pearson', 0),
                    'calibration_95_coverage': uq_results.get('calibration', {}).get('coverage', [0, 0, 0, 0])[-1] if uq_results.get('calibration') else 0
                }
            }
    ...
    def _run_expanded_hpo(self) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Target R² ≥ 0.5: {'ACHIEVED' if hpo_results['target_achieved'] else 'APPROACHING'}")
    ...
    def _train_ensemble_models(self) -> List[nn.Module]:
        ...
        if not trial_results:
            ...
            top_trials = [{'parameters': self.hpo_results['best_params'], 'r2_score': self.hpo_results['best_value']}]
        else:
            # Sort by R² and take top-k
            sorted_trials = sorted(trial_results, key=lambda x: x['r2_score'], reverse=True)
            top_trials = sorted_trials[:self.ensemble_size]
        
        ensemble_models = []
        
        for i, trial in enumerate(top_trials):
            logger.info(f"🎭 Training ensemble model {i+1}/{len(top_trials)} (R²: {trial['r2_score']:.4f})")
            
            try:
                model = self._train_single_model(trial['parameters'], f"ensemble_model_{i}")
    ...
    def _train_single_model(self, params: Dict[str, Any], model_name: str) -> nn.Module:
        ...
        dataset = PolymerDataset(
            ...
            smiles_column='canonical_smiles' if 'canonical_smiles' in pd.read_csv(self.dataset_path).columns else 'smiles',
            target_column='Tg',
        ...
        model = PolymerGCN(
            ...
            hidden_dims=[params.get('hidden_dims', 256)] * params.get('num_gcn_layers', 3),
            output_dim=1,
    ...
    def _run_uncertainty_quantification(self) -> Dict[str, Any]:
        ...
        logger.info(f"🎲 Running UQ analysis with {len(self.ensemble_models)} models...")
        ...
        logger.info(f"   Error-Uncertainty Correlation: {uq_results['calibration']['error_correlation']['pearson']:.3f}")
    ...
    def _final_validation(self) -> Dict[str, Any]:
        ...
        predictions, uncertainties, targets = ensemble.predict(test_loader, return_variance=True)
        ...
        logger.info(f"   Target R² ≥ 0.5: {'✅ ACHIEVED' if r2 >= 0.5 else '⚠️  APPROACHING'}")
    ...
    def _save_pipeline_results(self):
        ...
        # Save main results
        results_file = self.results_dir / f"week2_pipeline_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _print_final_summary(self):
        ...
        print(f"   🎯 Target R² ≥ 0.5: {'✅ ACHIEVED' if results['pipeline_info']['target_achieved'] else '⚠️  APPROACHING'}")
        print(f"   📈 Ensemble Improvement: +{results['ensemble_performance']['r2_improvement']:.4f}")
        
        print(f"\n🔍 HPO OPTIMIZATION:")
        print(f"   🧪 Total Trials: {results['hpo_results']['n_trials']}")
        print(f"   ✅ Completed: {results['hpo_results']['study_stats']['completed_trials']}")
        print(f"   ⏱️  Time: {results['hpo_results']['optimization_time_hours']:.2f} hours")
        
        print(f"\n🎲 UNCERTAINTY QUANTIFICATION:")
        uq_corr = results['uncertainty_quantification'].get('calibration', {}).get('error_correlation', {}).get('pearson', 0)
        uq_coverage = results['uncertainty_quantification'].get('calibration', {}).get('coverage', [0])[-1]
...
def main():
    ...
    parser.add_argument('--results_dir', type=str, default='results/week2_pipeline',
