        ...
        'standard': {
            ...
            'parameter_grids': {
                'random_search': {
                    ...
                    'hidden_dims': [[64, 32], [128, 64, 32], [256, 128, 64], [512, 256, 128]],
        ...
        'comprehensive': {
            ...
            'parameter_grids': {
                'aggressive_search': {
                    ...
                    'hidden_dims': [[32], [64], [128], [64, 32], [128, 64], [256, 128], [128, 64, 32], [256, 128, 64], [512, 256, 128], [1024, 512, 256, 128]],
...
def load_dataset_from_config(config: Dict[str, Any]) -> tuple:
    ...
    if not polymer_features:
        ...
        if search_type in parameter_grids:
            ...
            if grid.get('use_polymer_features', [False])[0]:  # If using polymer features
                polymer_features = {
                    'fingerprint_size': grid.get('fingerprint_size', [128])[0],
                    'log_scale_dp': True,
                    'include_chain_descriptors': grid.get('include_chain_descriptors', [True])[0],
                    'include_complexity': grid.get('include_complexity', [True])[0],
                    'include_molecular_descriptors': grid.get('include_molecular_descriptors', [True])[0],
                    'cache_features': True
                }
                logger.info(f"Using polymer features from parameter grid: {polymer_features}")
    ...
    logger.info(f"Dataset loaded - Train+Val: {len(combined_data)}, Test: {len(test_dataset)}")
...
def run_hpo_from_config(config: Dict[str, Any], search_type: str = None):
    ...
    polymer_feature_dim = param_grid.get('polymer_feature_dim', [0])[0] if use_polymer_features else 0
    ...
    if 'test_metrics' in final_results and success_config:
        ...
        logger.info(f"R² ≥ {r2_target}: {'✅' if r2_success else '❌'} ({test_metrics['r2']:.4f})")
        logger.info(f"RMSE ≤ {rmse_target}: {'✅' if rmse_success else '❌'} ({test_metrics['rmse']:.4f})")
        logger.info(f"MAE ≤ {mae_target}: {'✅' if mae_success else '❌'} ({test_metrics['mae']:.4f})")
        logger.info(f"Overall: {'🎉 SUCCESS!' if all_success else '⚠️  Partially successful'}")
...
def main():
    parser = argparse.ArgumentParser(description='Configuration-based HPO for Polymer GCN')
    
    # Configuration options
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--config', type=str, help='Path to configuration YAML file')
    group.add_argument('--preset', type=str, choices=['quick', 'standard', 'comprehensive', 'enhanced_full'],
                      help='Use predefined configuration preset')
    
    # Override options
    parser.add_argument('--search_type', type=str, 
                       choices=['grid_search', 'random_search', 'conservative_search', 'aggressive_search'],
    ...
    try:
        ...
        try:
            ...
            hpo_dirs = [d for d in (results_dir / "hpo").iterdir() 
